...
export const sendDataToWorshipNight = async (
  data: TSendDataToEvent,
): Promise<{ status: 'done' }> => {
  try {
    await fetch(
    ...
    console.error(error);
    throw new Error('Ошибка в регистрации пользователя на Вечер Хвалы');
  }
};

export const findUniquePersonOfWorshipNight = async (
  invId: string,
): Promise<TBusinessPerson | undefined> => {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_STRAPI_URL}/worship-night-registers?filters[personId][$eq]=${invId}`,
    );
...
export const updateWorshipNightPersonStatus = async (
  userId: number,
  newStatus: 'payed' | 'notPayed' | 'pending',
) => {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_STRAPI_URL}/worship-night-registers/${userId}`,
      {
        method: 'PUT',
...
export const deleteWorshipNightPerson = async (userId: number) => {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_STRAPI_URL}/worship-night-registers/${userId}`,
      {

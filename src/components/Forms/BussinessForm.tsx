...
import { TBussinesPaymentForm } from "@/interfaces/forms";
...
export function BussinessForm({ slug }: BussinessFormProps) {
  const { register, handleSubmit, isValid, errors, price, eventData } = useEventsForms<TBussinesPaymentForm>({ slug: slug });
  ...
  return (
    <StandartForm handleSubmit={handleSubmit} isValid={isValid} onSubmit={submit} price={price}>
      <InputList register={register} errors={errors} />
      <label className={styles.label}>
        <input
          {...register('career', { required: 'Опишите пожалуйста ваш род деятельности' })}
          placeholder='Род деятельности'
          className={styles.input}
          type="text" />
        {errors['career' as keyof TBussinesPaymentForm] &&
          <span className={styles.error}>{`${errors['career' as keyof TBussinesPaymentForm]?.message}`}</span>}
      </label>
